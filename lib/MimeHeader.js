	...
	module.exports = new Class( {
		...
		, init: function( text ){
			...
			// remove new lines contained inside the headers ( no each header is on a separate line )
			this.headerText = "\n" + text.replace( /(?:\n|\n\r|\r\n)\s+/gi, " " ).replace( /\s{2,}/g, " " ) + "\n";

			log.warn( "new header instance!" );
			log( this.headerText );
		}
		...
		, get: function( id, parsed ){
			...
			if ( h ){
				if ( parsed ) return this._headerFromText( h[ 1 ] );
				else return h[ 1 ];
			}
