	...
	var MimeMultiPart = module.exports = new Class( {
		...
		, init: function( options ){
			...
			if ( options && type.object( options.headers ) ){
		...
		, __getHeaders: function(){
			...
			log.error( "reading headers", this.offset );
			...
			if ( result = reg.exec( this.buffered ) ){
				...
				log.error( "headers found", this.offset );
			...
			if ( this.headersScanned ){
				...
				if ( this.currentPart ){
					...
					if ( this.boundary ){
						...
						log.debug( this.offset, this.buffered.substr( this.offset, 100 ) );

						// find next boundary at the end of the current part
						if ( result = this.boundary.exec( this.buffered ) ){							
							log.info( "received boudary end" );

							log.info( this.offset, result.index, this.buffered.substring( this.offset, result.index ) );
							...
							log.info( "not received boundary end", this.offset, this.buffered.length - this.boundaryMaxLength );
					...
					/*if ( this.boundary ){
						log.debug( "looking for next multipart boundary ..." );
						// set offset to after the next boundary
						this.boundary.lastIndex = this.offset;

						if ( result = this.boundary.exec( this.buffered ) ){
							log( "found ..." );
							log.warn( "offset", this.offset );
							this.offset = result.index + result[ 1 ].length;
							log.warn( "offset", this.offset );
						}
						else {
